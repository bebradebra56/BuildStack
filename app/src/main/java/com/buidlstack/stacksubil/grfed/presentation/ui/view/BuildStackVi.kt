...
class BuildStackVi(
    private val buildStackContext: Context,
    private val buildStackCallback: BuildStackCallBack,
    private val buildStackWindow: Window
) : WebView(buildStackContext) {
    private var buildStackFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun buildStackSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.buildStackFileChooserHandler = handler
    }
    ...
    init {
        ...
        webSettings.apply {
            ...
            userAgentString =
                WebSettings.getDefaultUserAgent(buildStackContext).replace("; wv)", "")
                    .replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        buildStackContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(
                            buildStackContext,
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("ninecasino") == true) {
                    BuildStackApplication.buildStackInputMode =
                        WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(
                        BuildStackApplication.BUILD_STACK_MAIN_TAG,
                        "onPageFinished : ${BuildStackApplication.buildStackInputMode}"
                    )
                    buildStackWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    buildStackEnableStableInputScroll()
                } else {
                    BuildStackApplication.buildStackInputMode =
                    ...
                    buildStackWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                buildStackFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            ...
            override fun onCreateWindow(
                ...
                buildStackHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    private fun buildStackEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    fun buildStackFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun buildStackHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView =
                BuildStackVi(buildStackContext, buildStackCallback, buildStackWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            buildStackCallback.buildStackHandleCreateWebWindowRequest(windowWebView)
        }
    }
